"use client";

import { useState, useEffect } from "react";
import { Shield, AlertCircle, Pill, Filter, ChevronDown, Phone, Snowflake, Thermometer, Home, Heart, Clock, Search, Star, Truck, CheckCircle, Droplets, Baby, Stethoscope, Users } from "lucide-react";
import MedicineCard from "@/app/components/MedicineCard";

const pregnancyMedicines = [
  {
    id: 1,
    name: "Folic Acid 5mg",
    type: "Pregnancy Vitamin",
    storage: "Room Temperature",
    price: 50,
    prescription: false,
    image: "/images/pregnancy/folic-acid.jpg",
    description: "Folic Acid is essential for preventing neural tube defects in developing babies. Recommended for all women planning pregnancy and during the first trimester.",
    rating: 4.8,
    brand: "Mankind",
    dosage: "5mg",
    quantity: "30 tablets",
    discount: 60,
    isHot: true,
    stock: "In Stock",
    delivery: "Same day",
    category: "vitamins"
  },
  {
    id: 2,
    name: "Iron Sucrose Injection",
    type: "Iron Supplement",
    storage: "Room Temperature",
    price: 850,
    prescription: true,
    image: "/images/pregnancy/iron-injection.jpg",
    description: "Used for treating iron deficiency anemia in pregnancy when oral iron is not tolerated or effective. Administered by healthcare professionals.",
    rating: 4.6,
    brand: "Emcure",
    dosage: "100mg/5mL",
    quantity: "1 vial",
    isNew: true,
    stock: "In Stock",
    delivery: "24 hours",
    category: "injections"
  },
  {
    id: 3,
    name: "Calcium + Vitamin D3",
    type: "Bone Health",
    storage: "Room Temperature",
    price: 120,
    prescription: false,
    image: "/images/pregnancy/calcium-d3.jpg",
    description: "Essential for fetal bone development and preventing maternal bone loss during pregnancy and breastfeeding.",
    rating: 4.7,
    brand: "USV",
    dosage: "500mg + 250IU",
    quantity: "30 tablets",
    discount: 150,
    isSale: true,
    stock: "In Stock",
    delivery: "Same day",
    category: "vitamins"
  },
  {
    id: 4,
    name: "Dydrogesterone",
    type: "Hormone Support",
    storage: "Room Temperature",
    price: 350,
    prescription: true,
    image: "/images/pregnancy/dydrogesterone.jpg",
    description: "Used for threatened miscarriage, recurrent miscarriage, and luteal phase support in assisted reproductive technology.",
    rating: 4.5,
    brand: "Sun Pharma",
    dosage: "10mg",
    quantity: "30 tablets",
    discount: 400,
    isHot: true,
    stock: "In Stock",
    delivery: "24 hours",
    category: "hormones"
  },
  {
    id: 5,
    name: "Low Molecular Weight Heparin",
    type: "Blood Thinner",
    storage: "2-8°C",
    price: 1200,
    prescription: true,
    image: "/images/pregnancy/heparin.jpg",
    description: "Used for preventing and treating blood clots during pregnancy in women with thrombophilia or previous thrombosis.",
    rating: 4.4,
    brand: "Novartis",
    dosage: "40mg/0.4mL",
    quantity: "1 prefilled syringe",
    stock: "In Stock",
    delivery: "48 hours",
    category: "injections"
  },
  {
    id: 6,
    name: "Multivitamin Prenatal",
    type: "Complete Supplement",
    storage: "Room Temperature",
    price: 450,
    prescription: false,
    image: "/images/pregnancy/prenatal-vitamin.jpg",
    description: "Comprehensive prenatal vitamin containing all essential vitamins and minerals required for healthy pregnancy and fetal development.",
    rating: 4.9,
    brand: "Cipla",
    dosage: "Once daily",
    quantity: "30 capsules",
    isNew: true,
    stock: "In Stock",
    delivery: "Same day",
    category: "vitamins"
  },
  {
    id: 7,
    name: "Metformin 500mg",
    type: "Blood Sugar Control",
    storage: "Room Temperature",
    price: 80,
    prescription: true,
    image: "/images/pregnancy/metformin.jpg",
    description: "Used for managing gestational diabetes and PCOS during pregnancy. Helps control blood sugar levels safely.",
    rating: 4.3,
    brand: "Dr. Reddy's",
    dosage: "500mg",
    quantity: "30 tablets",
    discount: 100,
    isSale: true,
    stock: "In Stock",
    delivery: "Same day",
    category: "diabetes"
  },
  {
    id: 8,
    name: "Anti-D Immunoglobulin",
    type: "Rh Incompatibility",
    storage: "2-8°C",
    price: 2800,
    prescription: true,
    image: "/images/pregnancy/anti-d.jpg",
    description: "Given to Rh-negative mothers to prevent Rh sensitization during pregnancy and after delivery of Rh-positive baby.",
    rating: 4.8,
    brand: "Bharat Serums",
    dosage: "300mcg",
    quantity: "1 vial",
    isHot: true,
    stock: "In Stock",
    delivery: "72 hours",
    category: "injections"
  }
];

const heroSlides = [
  {
    title: "Safe Pregnancy Medicines",
    subtitle: "Doctor Recommended • Pregnancy Safe • 24/7 Support",
    image: "https://images.unsplash.com/photo-1512672378591-6d6bd0b16d66?auto=format&fit=crop&w=1200&q=80"
  },
  {
    title: "Essential Pregnancy Care",
    subtitle: "Prenatal Vitamins • Iron Supplements • Hormone Support",
    image: "https://images.unsplash.com/photo-1551601651-2a8555f1a136?auto=format&fit=crop&w=1200&q=80"
  },
  {
    title: "Mother & Baby Wellness",
    subtitle: "Fetal Development • Maternal Health • Nutritional Support",
    image: "https://images.unsplash.com/photo-1582750433449-648ed127bb54?auto=format&fit=crop&w=1200&q=80"
  },
  {
    title: "Trusted Pregnancy Brands",
    subtitle: "Cipla • Sun Pharma • Mankind • Novartis • Emcure",
    image: "https://images.unsplash.com/photo-1631549916768-4119b2e5f926?auto=format&fit=crop&w=1200&q=80"
  }
];

const pregnancyCategories = [
  { id: "all", name: "All Medicines", icon: "💊", count: 8 },
  { id: "vitamins", name: "Prenatal Vitamins", icon: "🅰️", count: 3 },
  { id: "injections", name: "Injections", icon: "💉", count: 3 },
  { id: "hormones", name: "Hormone Support", icon: "⚖️", count: 1 },
  { id: "diabetes", name: "Diabetes Care", icon: "🩸", count: 1 }
];

const trimesters = [
  { id: "all", name: "All Trimesters", icon: "👶", count: 8 },
  { id: "first", name: "First Trimester", icon: "1️⃣", count: 4 },
  { id: "second", name: "Second Trimester", icon: "2️⃣", count: 3 },
  { id: "third", name: "Third Trimester", icon: "3️⃣", count: 3 },
  { id: "postnatal", name: "Postnatal Care", icon: "🤱", count: 2 }
];

export default function PregnancyPage() {
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");
  const [selectedCategory, setSelectedCategory] = useState<string>("all");
  const [selectedTrimester, setSelectedTrimester] = useState<string>("all");
  const [sortBy, setSortBy] = useState<string>("featured");
  const [showMobileFilters, setShowMobileFilters] = useState(false);
  const [currentSlide, setCurrentSlide] = useState(0);
  const [searchQuery, setSearchQuery] = useState("");

  // Auto slide every 5 seconds
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % heroSlides.length);
    }, 5000);
    return () => clearInterval(interval);
  }, []);

  const filteredMedicines = pregnancyMedicines.filter(medicine => {
    if (selectedCategory !== "all" && medicine.category !== selectedCategory) return false;
    return true;
  });

  const nextSlide = () => {
    setCurrentSlide((prev) => (prev + 1) % heroSlides.length);
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + heroSlides.length) % heroSlides.length);
  };

  const handleViewDetails = (id: number) => {
    console.log("View details for medicine ID:", id);
  };

  // Sort medicines based on sortBy
  const sortedMedicines = [...filteredMedicines].sort((a, b) => {
    switch (sortBy) {
      case "price-low":
        return a.price - b.price;
      case "price-high":
        return b.price - a.price;
      case "rating":
        return b.rating - a.rating;
      case "newest":
        return b.id - a.id;
      default:
        return 0;
    }
  });

  return (
    <div className="min-h-screen bg-white">
      {/* Hero Carousel - Blue Theme */}
      <div className="relative h-[50vh] sm:h-[55vh] md:h-[60vh] overflow-hidden">
        {heroSlides.map((slide, index) => (
          <div
            key={index}
            className={`absolute inset-0 transition-opacity duration-500 ${index === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
          >
            {/* Background Image */}
            <img 
              src={slide.image} 
              alt=""
              className="w-full h-full object-cover"
            />

            {/* Blue Overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-blue-900/80 via-blue-800/60 to-transparent"></div>

            {/* Content */}
            <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8">
              <div className="max-w-4xl mx-auto">
                {/* Badge - Yellow on Blue */}
                <div className="inline-flex items-center gap-1 bg-yellow-500 px-2 py-1 rounded-full mb-3 max-w-max">
                  <Heart className="w-3 h-3 text-white" />
                  <span className="text-xs font-bold text-white">PREGNANCY CARE SPECIALIST</span>
                </div>

                {/* Title */}
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2 leading-tight">
                  {slide.title}
                </h1>
                
                {/* Subtitle */}
                <p className="text-sm sm:text-base text-white/90 mb-4 max-w-md">
                  {slide.subtitle}
                </p>
                
                {/* Search Bar */}
                <div className="max-w-md">
                  <div className="flex flex-col sm:flex-row gap-2">
                    <div className="flex-1">
                      <input
                        type="text"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder="Search pregnancy medicines..."
                        className="w-full py-2.5 px-4 rounded-lg text-gray-800 focus:outline-none text-sm"
                      />
                    </div>
                    <button className="bg-blue-700 hover:bg-blue-800 text-white px-4 py-2.5 rounded-lg font-bold text-sm flex items-center justify-center gap-2 min-w-[100px]">
                      <Search className="w-4 h-4" />
                      Search
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>
        ))}

        {/* Navigation Buttons */}
        <button 
          onClick={prevSlide}
          className="absolute left-4 top-1/2 transform -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all z-20"
        >
          ←
        </button>
        <button 
          onClick={nextSlide}
          className="absolute right-4 top-1/2 transform -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all z-20"
        >
          →
        </button>
      </div>

      {/* Safety Banner - Yellow Theme */}
      <div className="container mx-auto px-4 -mt-4 relative z-10">
        <div className="bg-gradient-to-r from-yellow-50 to-yellow-100 border-2 border-yellow-300 rounded-xl p-4 shadow-lg">
          <div className="flex items-start gap-3">
            <Shield className="w-6 h-6 text-yellow-600 flex-shrink-0" />
            <div>
              <h3 className="font-bold text-yellow-800 text-lg mb-1">Pregnancy Safety First</h3>
              <p className="text-yellow-700 text-sm">
                All medicines should be taken under medical supervision during pregnancy. 
                Consult your obstetrician before starting any medication. Some medicines require special monitoring.
              </p>
            </div>
          </div>
        </div>
      </div>

      {/* Main Content */}
      <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-8">
        {/* Mobile Filter Toggle */}
        <div className="lg:hidden mb-4">
          <button
            onClick={() => setShowMobileFilters(!showMobileFilters)}
            className="flex items-center justify-between w-full p-3 bg-white rounded-lg shadow-sm border border-gray-200"
          >
            <div className="flex items-center gap-2">
              <Filter className="w-4 h-4" />
              <span className="font-medium">Filters & Sort</span>
            </div>
            <ChevronDown className={`w-4 h-4 transition-transform ${showMobileFilters ? 'rotate-180' : ''}`} />
          </button>
        </div>

        {/* Category Filters - Blue Yellow Theme */}
        <div className={`${showMobileFilters ? 'block' : 'hidden lg:block'} mb-6 sm:mb-8`}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 sm:mb-6">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">Browse Pregnancy Medicines</h2>
              <p className="text-sm text-gray-600">Filter by category or trimester</p>
            </div>
            
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              {/* Sort By */}
              <div className="relative w-full sm:w-auto">
                <select 
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value)}
                  className="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none"
                >
                  <option value="featured">Featured</option>
                  <option value="price-low">Price: Low to High</option>
                  <option value="price-high">Price: High to Low</option>
                  <option value="rating">Highest Rating</option>
                  <option value="newest">Newest</option>
                </select>
                <ChevronDown className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-500 pointer-events-none" />
              </div>

              {/* View Toggle */}
              <div className="flex items-center gap-1 bg-blue-50 p-1 rounded-lg">
                <button
                  onClick={() => setViewMode("grid")}
                  className={`p-2 rounded-md transition-all text-sm ${
                    viewMode === "grid" 
                      ? "bg-blue-600 text-white shadow-sm" 
                      : "text-gray-600 hover:text-gray-900"
                  }`}
                >
                  Grid
                </button>
                <button
                  onClick={() => setViewMode("list")}
                  className={`p-2 rounded-md transition-all text-sm ${
                    viewMode === "list" 
                      ? "bg-blue-600 text-white shadow-sm" 
                      : "text-gray-600 hover:text-gray-900"
                  }`}
                >
                  List
                </button>
              </div>
            </div>
          </div>

          
        </div>


        {/* Products Grid/List */}
        <div className="mb-8 sm:mb-12">
          <div className={`${viewMode === "grid" 
            ? "grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" 
            : "space-y-4"
          }`}>
            {sortedMedicines.map((medicine) => (
              <MedicineCard
                key={medicine.id}
                product={medicine}
                type={viewMode}
                onViewDetails={handleViewDetails}
              />
            ))}
          </div>

          {/* No Results */}
          {sortedMedicines.length === 0 && (
            <div className="text-center py-10 sm:py-12">
              <div className="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 bg-blue-50 rounded-full flex items-center justify-center">
                <Heart className="w-8 h-8 sm:w-10 sm:h-10 text-blue-400" />
              </div>
              <h3 className="text-lg sm:text-xl font-bold text-gray-700 mb-2">No pregnancy medicines found</h3>
              <p className="text-sm text-gray-600 max-w-md mx-auto px-4">
                Try adjusting your filters or contact our pregnancy care team
              </p>
            </div>
          )}
        </div>

        {/* Information Cards - Blue Yellow Theme */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-8 sm:mb-12">
          {/* Safety Guide - Blue */}
          <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-4 sm:p-6 border border-blue-200 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-600 to-blue-700 rounded-lg flex items-center justify-center">
                <Shield className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              </div>
              <div>
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-1">Pregnancy Safety</h2>
                <p className="text-sm text-blue-600">Important guidelines for medicine use during pregnancy</p>
              </div>
            </div>
            
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-3 bg-white rounded-lg border border-blue-200">
                <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
                  <CheckCircle className="w-4 h-4 text-blue-600" />
                </div>
                <div>
                  <h4 className="font-bold text-gray-800 mb-1 text-sm">Doctor Consultation</h4>
                  <p className="text-gray-600 text-xs">• Always consult obstetrician</p>
                  <p className="text-gray-600 text-xs">• Never self-medicate</p>
                  <p className="text-gray-600 text-xs">• Regular check-ups essential</p>
                </div>
              </div>
              
              <div className="flex items-start gap-3 p-3 bg-white rounded-lg border border-blue-200">
                <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
                  <Clock className="w-4 h-4 text-blue-600" />
                </div>
                <div>
                  <h4 className="font-bold text-gray-800 mb-1 text-sm">Timing Matters</h4>
                  <p className="text-gray-600 text-xs">• Some medicines trimester-specific</p>
                  <p className="text-gray-600 text-xs">• Follow prescribed schedule</p>
                  <p className="text-gray-600 text-xs">• Don't skip doses</p>
                </div>
              </div>
            </div>
          </div>

          {/* Nutritional Information - Yellow */}
          <div className="bg-gradient-to-br from-yellow-50 to-white rounded-xl p-4 sm:p-6 border border-yellow-200 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-yellow-600 to-yellow-500 rounded-lg flex items-center justify-center">
                <Baby className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              </div>
              <div>
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-1">Essential Nutrients</h2>
                <p className="text-sm text-yellow-600">Key supplements for healthy pregnancy</p>
              </div>
            </div>
            
            <div className="space-y-2">
              {[
                "Folic Acid: Prevents neural tube defects",
                "Iron: Prevents anemia, supports growth",
                "Calcium: Builds baby's bones",
                "Vitamin D: Aids calcium absorption",
                "DHA: Brain and eye development",
                "Iodine: Thyroid function, brain development"
              ].map((item, index) => (
                <div key={index} className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 bg-yellow-500 rounded-full flex-shrink-0 mt-1.5"></div>
                  <span className="text-sm text-gray-700">{item}</span>
                </div>
              ))}
            </div>

            {/* Warning Box */}
            <div className="mt-4 p-3 bg-yellow-100 border border-yellow-300 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                <AlertCircle className="w-4 h-4 text-yellow-700" />
                <span className="font-bold text-yellow-800 text-sm">Important Notice</span>
              </div>
              <p className="text-xs text-yellow-700">
                Some supplements require blood tests and monitoring. 
                Overdose of certain vitamins can be harmful during pregnancy.
              </p>
            </div>
          </div>
        </div>

        {/* Pregnancy Timeline */}
        <div className="mb-8 sm:mb-12">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Pregnancy Timeline & Care</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              {
                trimester: "First Trimester",
                weeks: "1-12 weeks",
                keyMedicines: ["Folic Acid", "Anti-nausea", "Thyroid meds"],
                icon: "🌱",
                color: "bg-blue-50"
              },
              {
                trimester: "Second Trimester",
                weeks: "13-27 weeks",
                keyMedicines: ["Iron", "Calcium", "Multivitamins"],
                icon: "🌿",
                color: "bg-yellow-50"
              },
              {
                trimester: "Third Trimester",
                weeks: "28-40 weeks",
                keyMedicines: ["Anti-D", "LMWH", "Tocolytics"],
                icon: "🌳",
                color: "bg-blue-50"
              }
            ].map((stage, index) => (
              <div key={index} className={`${stage.color} rounded-xl p-4 border border-gray-200`}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-xl">
                    {stage.icon}
                  </div>
                  <div>
                    <h3 className="font-bold text-gray-900">{stage.trimester}</h3>
                    <p className="text-sm text-gray-600">{stage.weeks}</p>
                  </div>
                </div>
                <div className="space-y-2">
                  <p className="text-sm font-medium text-gray-700">Key Medicines:</p>
                  <div className="flex flex-wrap gap-1">
                    {stage.keyMedicines.map((medicine, idx) => (
                      <span key={idx} className="px-2 py-1 bg-white/70 rounded text-xs text-gray-700">
                        {medicine}
                      </span>
                    ))}
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>

        {/* Popular Brands */}
        <div className="mb-8 sm:mb-12">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Trusted Pregnancy Care Brands</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4">
            {[
              { name: "Cipla", logo: "🏥", color: "bg-blue-50" },
              { name: "Mankind", logo: "👨‍👩‍👧‍👦", color: "bg-yellow-50" },
              { name: "Sun Pharma", logo: "☀️", color: "bg-blue-50" },
              { name: "Emcure", logo: "💊", color: "bg-yellow-50" },
              { name: "USV", logo: "🏢", color: "bg-blue-50" },
              { name: "Dr. Reddy's", logo: "👨‍⚕️", color: "bg-yellow-50" },
              { name: "Novartis", logo: "🔬", color: "bg-blue-50" },
              { name: "Bharat Serums", logo: "💉", color: "bg-yellow-50" },
              { name: "Pfizer", logo: "🏭", color: "bg-blue-50" },
              { name: "GSK", logo: "🌍", color: "bg-yellow-50" }
            ].map((brand, index) => (
              <div 
                key={index} 
                className="bg-white border border-gray-200 rounded-xl p-4 text-center hover:shadow-md transition-all hover:-translate-y-1 cursor-pointer"
              >
                <div className={`w-12 h-12 mx-auto mb-3 ${brand.color} rounded-full flex items-center justify-center text-xl`}>
                  {brand.logo}
                </div>
                <span className="font-medium text-gray-800 text-sm">{brand.name}</span>
              </div>
            ))}
          </div>
        </div>

        {/* CTA Section - Blue Yellow */}
        <div className="bg-gradient-to-r from-blue-700 to-blue-800 rounded-xl p-4 sm:p-6 text-center text-white shadow-lg">
          <div className="inline-flex items-center gap-2 bg-yellow-500/20 backdrop-blur-sm px-3 py-1.5 rounded-full mb-4">
            <Phone className="w-3.5 h-3.5 text-yellow-400" />
            <span className="text-xs sm:text-sm font-bold">24/7 Pregnancy Support</span>
          </div>
          
          <h2 className="text-lg sm:text-xl font-bold mb-3">Need Obstetrician Consultation?</h2>
          <p className="text-sm sm:text-base mb-4 text-blue-100 max-w-lg mx-auto">
            Connect with our panel of certified obstetricians for personalized pregnancy care, 
            medicine guidance, and regular monitoring.
          </p>
          <div className="flex flex-col xs:flex-row gap-3 justify-center">
            <button className="bg-white text-blue-700 hover:bg-gray-100 px-4 py-2.5 rounded-lg font-bold hover:scale-105 transition-all flex items-center justify-center gap-2 text-sm">
              <span className="text-base">👩‍⚕️</span>
              Consult Obstetrician
            </button>
            <button className="bg-white/10 backdrop-blur-lg border-2 border-white/30 hover:bg-white/20 px-4 py-2.5 rounded-lg font-bold hover:scale-105 transition-all flex items-center justify-center gap-2 text-sm">
              <Phone className="w-4 h-4" />
              Emergency: 1800-123-4567
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}